...
监控组件覆盖指标、仪表盘、链路追踪、告警基础和通用遥测管线；应用日志采集、聚合查询、解析转换和保留策略放在日志管理与分析分类，应用异常收集、崩溃报告和会话回放辅助排障放在错误跟踪与异常上报分类，LLM 调用追踪、Prompt 版本和评估数据集放在 LLM 可观测性与评估分类，对外公开服务状态、事故记录和维护窗口放在状态页与事故沟通分类。
