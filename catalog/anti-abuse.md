...
反滥用与验证码组件覆盖验证码、人机验证、登录防爆破、表单防刷、接口限流、反垃圾和基础机器人防护；依赖漏洞扫描放在安全扫描分类，入口代理限流放在 API 网关分类，字段合法性校验放在表单与校验分类。反滥用策略通常需要和认证、评论、论坛、支付、Webhook、监控和审计日志一起设计，不能只接一个验证码控件就结束。
