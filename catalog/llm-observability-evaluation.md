...
LLM 可观测性与评估组件覆盖提示词版本、模型调用追踪、Token 与成本分析、数据集、离线评估、在线反馈、RAG 质量评估和红队测试。它解决“AI 应用效果是否稳定、哪里出错、上线前后如何评估”的问题；LLM/RAG 编排和 Agent 放在 AI 分类，模型托管、推理服务和模型网关放在模型服务与推理网关分类，通用指标和链路追踪放在监控分类，普通单元测试和端到端测试放在测试与质量分类。
