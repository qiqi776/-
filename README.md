...
1. 先列出项目需要的能力：认证、授权策略、审计日志、UI、移动端、内部工具、表单与校验、国际化、后端 API、API 文档、API 网关、边缘交付、服务网格、工程化、数据库、数据库迁移、数据管道、流处理、数据仓库、数据质量、数据治理、备份恢复、文件存储、文档处理、媒体处理、邮件通知、通知触达、Webhook 投递、客户支持、CRM、营销自动化、电商、项目管理、AI、内容管理、富文本编辑、搜索、缓存、消息队列、后台任务、工作流、实时通信、产品分析、图表可视化、地图地理、功能开关、配置密钥、隐私合规、支付、账单与发票、基础设施即代码、容器镜像、部署、CI/CD、测试、安全扫描、监控。
